...
なお `gh pr create` の直後は timeline への反映に数秒のラグがあり、一度きりの問い合わせだと
PR が正しく作られているのに nil が返って誤って `Blocked` に落ちる。
そのため **2 秒間隔で最大 3 回リトライ**してから「PR 無し」と判断する。
リトライ中の一時的な API エラーもラグと同じ扱いで吸収する。
