...
| 3 | 🚧 **In Progress** | **🤖 Agent** | Ready 検知 / 検証失敗の差し戻し / PRコメント検知 / Blocked助言コメント検知 | **実装（Agent が稼働中の唯一のレーン）**<br>1. ブランチ作成 ＆ 実装<br>2. ローカルテスト実行<br>3. PR 作成 / 更新（**Issue と同じリポジトリに**、`Closes #Issue番号`） | PR 作成・更新完了で `🔍 Verifying`<br>技術的スタックで `⏸ Blocked` |
...
4. **タスク分割（親Issue / 子Issue）と手動クローズ**:
   - 次のいずれかに当てはまる要求は、Agent が複数の子Issue（Task）に分割し、親Issue から参照する（GitHub の sub-issues 機能を利用する）。
     - 1つのPRに収まらない規模である
     - **実装先が親Issueのリポジトリと異なる、または複数のリポジトリに跨る**
   - **子Issueは実装先のリポジトリに起票する**。sub-issue はリポジトリを跨いで登録できるため、実装先が別リポジトリでも親子関係は保たれる。
   - Issue と、それを解決する PR は **必ず同じリポジトリに置く**。他リポジトリに PR を作ってはならない。
...
### シナリオ7: 実装先が別リポジトリのケース

複数リポジトリを横断する運用では、要求の起票先（管理用リポジトリなど）と実装先が一致しないことがある。

1. Requester が管理用リポジトリに「監視対象のホストで systemd のメトリクスを取れるようにしたい」と起票（`📥 Inbox`）。
2. Agent が精緻化の過程で、変更が必要なのは別リポジトリ（インフラ定義側）だと判断する。
   **ここで実装に進んではならない**。PR を跨がせると、そのリポジトリの CI と品質ゲートで検証されないまま In Review に到達してしまう。
3. Agent は親Issueに全体仕様をまとめ、**実装先のリポジトリに子Issueを起票**して sub-issue として紐付け、Project に追加する（シナリオ6 と同じ手順）。
4. 以降は子Issueが実装先のリポジトリで完結して自走する。PR も `Closes #子Issue番号` で同じリポジトリ内に閉じる。
5. 親Issueは管理用リポジトリに静止し、人間が手動でクローズして `✅ Done` とする。

実装先のリポジトリがワーカーの監視対象に入っていない場合、子Issueは誰にも拾われない。監視対象の設定は運用側の前提条件である。
